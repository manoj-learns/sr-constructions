...
export default function OngoingForm({ initial = {}, onSave, onCancel, saving }) {
  const [form, setForm] = useState({
    name: initial.name || '',
    type: initial.type || 'Plot Layout',
    badge: initial.badge || 'Available',
  ...
  const onGalleryFile = async (e) => {
    ...
    try {
      const url = await uploadImage(file, 'ongoing/gallery', setGalleryProgress);
      setGallery((g) => [...g, url]);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (imgFile) {
      ...
      setCoverProgress(0);
      try {
        img = await uploadImage(imgFile, 'ongoing', setCoverProgress);
      } catch (err) {
        setUploading(false);
        setCoverProgress(0);
    ...
    const data = {
      name: form.name, type: form.type, badge: form.badge, badgeType: 'available',
      location: form.location, completion: form.completion, desc: form.desc, img,
    ...
    <form onSubmit={handleSubmit} style={fs.form}>
      <Section title="Basic Info">
        <Row>
          <Field label="Project Name" required>
            <input style={fs.input} value={form.name} onChange={set('name')} placeholder="e.g. Sai Ram Elite" required />
          </Field>
          <Field label="Type">
            <select style={fs.input} value={form.type} onChange={set('type')}>
        ...
        <Row>
          <Field label="Location" required>
            <input style={fs.input} value={form.location} onChange={set('location')} placeholder="e.g. Pendurthi, Visakhapatnam, AP" required />
          </Field>
          <Field label="Availability / Status">
            <input style={fs.input} value={form.completion} onChange={set('completion')} placeholder="e.g. 60 Plots Available" />
          </Field>
        </Row>
        <Row>
          <Field label="Badge Text">
            <input style={fs.input} value={form.badge} onChange={set('badge')} placeholder="e.g. Available" />
          </Field>
        </Row>
      ...
      <Section title="Highlights">
        <Field label="One highlight per line">
          <textarea style={{ ...fs.input, minHeight: 140, resize: 'vertical', fontFamily: 'monospace' }} value={form.highlights} onChange={set('highlights')} placeholder={'60 plots across 10 acres\nL.P. Number to be received shortly'} />
        </Field>
      </Section>

      <Section title="Specifications">
        <Field label="One spec per line — format: Label: Value">
          <textarea style={{ ...fs.input, minHeight: 120, resize: 'vertical', fontFamily: 'monospace' }} value={form.specs} onChange={set('specs')} placeholder={'Total Plots: 60\nTotal Area: 10 Acres\nStatus: Available'} />
        </Field>
      </Section>
